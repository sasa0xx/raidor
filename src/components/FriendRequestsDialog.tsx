  ...
  const fetchRequests = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        receiver:profiles!receiver_id(id, username, display_name)
      `)
      .eq("status", "pending");
              ...
              return (
                <div key={req.id} className="flex items-center justify-between p-2 rounded-lg border border-slate-100 dark:border-gray-800 bg-slate-50 dark:bg-gray-800/50">
                  ...
                  <div className="flex gap-1 shrink-0 ml-2">
                    {activeTab === "incoming" ? (
                      <>
                        <Button varient="ghost" className="small-btn px-2! py-1! text-xs" onClick={() => handleUpdateStatus(req.id, "accepted")}>Accept</Button>
                        <Button varient="ghost" className="small-btn px-2! py-1! border-0! text-xs text-red-500! hover:bg-red-50 dark:hover:bg-red-950" onClick={() => handleUpdateStatus(req.id, "rejected")}>Reject</Button>
                      </>
                    ) : (
